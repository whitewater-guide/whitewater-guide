...
BEGIN
  ...
  WITH upserted_section AS (
    ...
    VALUES (
      ...
      section ->> 'helpNeeded',
      (
        SELECT premium AND NOT ((section ->> 'verified') :: boolean)
        FROM rivers
        INNER JOIN regions on rivers.region_id = regions.id
        WHERE rivers.id = ((section -> 'river') ->> 'id') :: UUID
      ),
      section ->> 'importId',
      (section ->> 'verified') :: boolean,
      (section ->> 'license') :: JSONB,
      COALESCE(section ->> 'timezone', tz_from_lon_lat((section -> 'shape' -> 0 ->> 0) :: float, (section -> 'shape' -> 0 ->> 1) :: float)),
      lang
    )
    ON CONFLICT (id)
      DO UPDATE SET
        ...
        -- demo            = EXCLUDED.demo, -- demo is changed in admin mutation
        import_id       = EXCLUDED.import_id,
        license         = EXCLUDED.license,
        timezone        = EXCLUDED.timezone
    RETURNING id
  )
  INSERT INTO sections_translations (section_id, language, name, alt_names, description, copyright, season, flows_text)
    SELECT
      upserted_section.id,
      lang,
      section ->> 'name',
