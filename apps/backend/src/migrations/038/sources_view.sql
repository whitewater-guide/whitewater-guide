...
SELECT
  ...
  sources.created_at,
  sources.updated_at,
  COALESCE(sources_translations.name, eng.name, default_trans.name, 'Not translated') as name,
  COALESCE(sources_translations.terms_of_use, eng.terms_of_use, default_trans.terms_of_use) as terms_of_use
FROM langs
  CROSS JOIN sources
  LEFT OUTER JOIN sources_translations
          ON sources.id = sources_translations.source_id
  ...
  LEFT OUTER JOIN sources_translations eng
          ON sources.id = eng.source_id
            AND eng.language = 'en'
