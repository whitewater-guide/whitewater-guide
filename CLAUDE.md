...
whitewater.guide — a full-stack monorepo for a whitewater sports platform. Includes a Node.js GraphQL backend, React web admin app, React Native mobile app, and shared libraries. Licensed under CC BY-NC-SA 4.0.
...
# Check dependency version mismatches
pnpm deps:versions
```
...
```bash
# Build/test/typecheck a single package
...
cd packages/backend && pnpm test -- --testPathPattern=path/to/test
...
### Monorepo layout (pnpm workspaces in `packages/`)
...
| `mobile`                | React Native app (RN 0.72, react-navigation 6, Mapbox, Firebase, Paper UI)                                           |
...
- Backend resolvers generated to `packages/backend/src/apollo/resolvers.generated.ts`
- Frontend queries/mutations use `near-operation-file` preset (`.generated.ts` siblings next to `.gql` files)
- Mobile has an additional local schema (`packages/mobile/schema/mobile-local-schema.graphql`)
...
- Database models/types in `packages/backend/src/db/` with `Sql.*` namespace
...
- **TypeScript**: Strict mode, ESNext target, `strictFunctionTypes: false`
- **Formatting**: Prettier — 80 chars, single quotes, trailing commas, arrow parens
- **Secrets**: `.env.*` files encrypted with `git secret`, gitignored. Never commit unencrypted env files.
