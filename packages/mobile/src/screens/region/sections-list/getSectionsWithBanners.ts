...
import { Banner, BannerPlacement, Region, Section } from '../../../ww-commons';

const getSectionsWithBanners = (sections: Section[], region: Region | null, rowsPerScreen: number): Array<Section | Banner> => {
  if (!region) {
    return sections;
  }
  ...
  const banners = getBannersForPlacement(nodes!, BannerPlacement.MOBILE_SECTION_ROW, numBanners);
