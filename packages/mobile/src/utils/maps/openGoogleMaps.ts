...
export const openGoogleMaps = async (coordinates: Coordinate) => {
  let directionsURL = `https://www.google.com/maps/search/?api=1&query=${coordinates[1]},${coordinates[0]}`;
  if (Platform.OS === 'ios') {
    ...
    Firebase.analytics().setUserProperty('canOpenGoogleMaps', canOpenGoogleMaps ? 'true' : 'false');
  }
