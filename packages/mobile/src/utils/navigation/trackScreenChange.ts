...
export const trackScreenChange = (prev: NavigationState, next: NavigationState) => {
  ...
  if (currentScreenName && prevScreenName !== currentScreenName && currentScreen) {
    Firebase.analytics().setCurrentScreen(currentScreenName);
    Firebase.analytics().logEvent(`Screen_${currentScreenName}`, currentScreen.params);
  }
};
