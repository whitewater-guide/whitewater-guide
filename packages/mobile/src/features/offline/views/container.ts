...
const mapStateToProps = ({ offlineContent, network }: RootState): StateProps => ({
  ...
  inProgress:
    !!offlineContent.dialogRegion && offlineContent.dialogRegion.id === offlineContent.regionInProgress,
...
const mapDispatchToProps = (dispatch: Dispatch): DispatchProps => ({
  ...
  onDownload: (selection: OfflineCategorySelection) => dispatch(offlineContentActions.startDownload(selection)),
});
