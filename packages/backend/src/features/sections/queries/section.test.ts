...
describe('data', () => {
  it('should return simple data', async () => {
    const result = await runQuery(query, { id: NORWAY_SJOA_AMOT }, fakeContext(ADMIN));
    expect(result.errors).toBeUndefined();
...
describe('i18n', () => {
  ...
  it('should be able to specify language', async () => {
    const result = await runQuery(query, { id: NORWAY_SJOA_AMOT }, fakeContext(EDITOR_NO_EC, 'ru'));
    expect(result.errors).toBeUndefined();
  ...
  it('should fall back to english when not translated', async () => {
    const result = await runQuery(query, { id: NORWAY_SJOA_AMOT }, fakeContext(EDITOR_NO_EC, 'pt'));
    expect(result.errors).toBeUndefined();
    expect(result.data!.section).toMatchObject({
      name: 'Amot',
...
describe('premium access', () => {
  ...
  it('should return demo section description when premium region is not purchased', async () => {
    const result = await runQuery(descrQuery, { id: GEORGIA_BZHUZHA_QUALI }, fakeContext(EDITOR_NO_EC));
    expect(result.errors).toBeUndefined();
    expect(result).toHaveProperty('data.section.description', 'Bzhuzha Qualification description');
  });
