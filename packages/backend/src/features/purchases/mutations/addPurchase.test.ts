...
it('anon should fail', async () => {
  ...
  const result = await testAddPurchase({ purchase }, anonContext());
...
it('duplicate purchase should return false when for same user', async () => {
  ...
  const result = await testAddPurchase({ purchase }, fakeContext(TEST_USER));
...
it('duplicate purchase should return throw when for different user', async () => {
  ...
  const result = await testAddPurchase(
    { purchase },
...
describe('boomstarter', () => {
  ...
  describe('success', () => {
    ...
    beforeEach(async () => {
      ...
      result = await testAddPurchase({ purchase }, fakeContext(BOOM_USER_3500));
    });
