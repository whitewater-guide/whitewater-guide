    ...
    if (regionId) {
      ...
      query.with('region_banners_group', (qb) => {
        qb.table('banners_groups')
          .innerJoin('regions_groups', 'banners_groups.group_id', '=', 'regions_groups.group_id')
          .select('banners_groups.*')
          .where({ region_id: regionId });
      });
      query.leftJoin('region_banners_own', 'region_banners_own.banner_id', '=', 'banners.id');
      query.select(db().raw(`CASE WHEN region_banners_own.banner_id IS NOT NULL THEN 1 ELSE 0 END AS group_priority `));
      query.where((qb) => {
        qb
          .whereIn(
            'id',
