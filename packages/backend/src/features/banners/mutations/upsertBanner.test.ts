...
import { BANNERS, fileExistsInBucket, resetTestMinio, TEMP, TEMP_BUCKET_DIR } from '@minio';
import { ADMIN, EDITOR_GA_EC, TEST_USER } from '@seeds/01_users';
import { GROUP_LATIN } from '@seeds/03_groups';
import { REGION_GALICIA, REGION_NORWAY } from '@seeds/04_regions';
import { GALICIA_REGION_DESCR_BANNER, GALICIA_REGION_DESCR_BANNER2 } from '@seeds/14_banners';
import { anonContext, countRows, fakeContext, runQuery, UUID_REGEX } from '@test';
import { ApolloErrorCodes, BannerInput, BannerKind, BannerPlacement } from '@ww-commons';
...
beforeAll(async () => {
  [bBefore, brBefore, bgBefore] = await countRows(true, 'banners', 'banners_regions', 'banners_groups');
});
...
describe('resolvers chain', () => {
  ...
  it('editor should not pass', async () => {
    const result = await runQuery(mutation, { banner }, fakeContext(EDITOR_GA_EC));
    expect(result).toHaveGraphqlError(ApolloErrorCodes.FORBIDDEN);
  });
  ...
  it('should throw on invalid input', async () => {
    ...
    const result = await runQuery(mutation, { banner: invalidInput }, fakeContext(ADMIN));
...
describe('insert', () => {
  ...
  it('should add one more banner', async () => {
    ...
    const [bAfter, brAfter, bgAfter] = await countRows(false, 'banners', 'banners_regions', 'banners_groups');
    expect([bAfter - bBefore, brAfter - brBefore, bgAfter - bgBefore]).toEqual([1, 1, 1]);
  });
...
describe('update', () => {
  ...
  it('should return result', async () => {
    const result = await runQuery(mutation, { banner: input }, fakeContext(ADMIN));
    expect(result.errors).toBeUndefined();
    expect(result.data!.upsertBanner).toEqual({
      ...banner,
      id: GALICIA_REGION_DESCR_BANNER,
  ...
  it('should not change total number of banners', async () => {
    ...
    const [bAfter, brAfter, bgAfter] = await countRows(false, 'banners', 'banners_regions', 'banners_groups');
    expect([bAfter - bBefore, brAfter - brBefore, bgAfter - bgBefore]).toEqual([0, 0, 0]);
  });
...
describe('files', () => {
  ...
  it('test banner should exist in bucket', async () => {
    await expect(fileExistsInBucket(BANNERS, 'banner_4.jpg', '6a188ddefa676b8d60082f21be94d212'))
      .resolves.toBe(true);
  });
  ...
  it('should move file from temp to banners', async () => {
    const result = await runQuery(mutation, { banner: newImageBanner }, fakeContext(ADMIN));
    ...
    await expect(fileExistsInBucket(BANNERS, NEW_FILE_ID, 'cdd9bc2685a21655675f546c3d828d5f'))
      .resolves.toBe(true);
  });
  ...
  it('should store only filenames', async () => {
    ...
    const { source } = await db().table('banners')
      .select(['source'])
  ...
  it('should delete old image when new is uploaded', async () => {
    const result = await runQuery(mutation, { banner: oldImageBanner }, fakeContext(ADMIN));
    expect(result).not.toHaveGraphqlError();
    await expect(fileExistsInBucket(TEMP, 'banner_5.jpg'))
      .resolves.toBe(false);
    await expect(fileExistsInBucket(BANNERS, NEW_FILE_ID, 'cdd9bc2685a21655675f546c3d828d5f'))
      .resolves.toBe(true);
    await expect(fileExistsInBucket(BANNERS, 'banner_4.jpg'))
      .resolves.toBe(false);
  });

  it('should not delete old image when it is not changed', async () => {
    const result = await runQuery(mutation, { banner: oldBanner }, fakeContext(ADMIN));
    expect(result).not.toHaveGraphqlError();
    await expect(fileExistsInBucket(TEMP, 'banner_5.jpg'))
      .resolves.toBe(false);
    await expect(fileExistsInBucket(BANNERS, 'banner_4.jpg', '6a188ddefa676b8d60082f21be94d212'))
  ...
  it('should delete image when isImage changes to false', async () => {
    const result = await runQuery(mutation, { banner: oldUrlBanner }, fakeContext(ADMIN));
    expect(result).not.toHaveGraphqlError();
    await expect(fileExistsInBucket(TEMP, 'banner_5.jpg'))
      .resolves.toBe(false);
    await expect(fileExistsInBucket(BANNERS, 'banner_4.jpg'))
...
it('should sanitize input', async () => {
  ...
  const result = await runQuery(mutation, { banner: dirty }, fakeContext(ADMIN));
  expect(result).toHaveProperty('data.upsertBanner.name', "it's a \\ $1 slash with . ?");
});
