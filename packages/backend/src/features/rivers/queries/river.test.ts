...
it('should paginate sections', async () => {
  const result = await runQuery(query, { id: RIVER_BZHUZHA, sectionsPage: { limit: 1, offset: 1 } });
  expect(result.data!.river.sections.count).toBe(3);
  expect(result.data!.river.sections.nodes).toHaveLength(1);
  expect(result).toHaveProperty('data.river.sections.nodes.0.name', 'Long Race');
});
