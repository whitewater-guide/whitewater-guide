...
const hasGraphlError = (received: any, code?: string, message?: string): boolean => {
  ...
  const hasNoData = !received.data || Object.values(received.data).every((v: any) => v === null);
  return hasError && hasNoData;
};

const hasGraphqlValidationError = (received: any): boolean => {
  return hasGraphlError(received, ApolloErrorCodes.BAD_USER_INPUT)
