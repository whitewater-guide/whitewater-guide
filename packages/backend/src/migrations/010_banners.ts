...
export const up = async (db: Knex) => {
  ...
  await db.schema.createTable('banners_regions', (table) => {
    table.uuid('banner_id').notNullable().references('id').inTable('banners').onDelete('CASCADE');
  ...
  await runSqlFile(db, './dist/migrations/010/upsert_banner.sql');
...
export const down = async (db: Knex) => {
  ...
  await db.schema.raw('DROP FUNCTION IF EXISTS upsert_banner(banner JSON) CASCADE');
