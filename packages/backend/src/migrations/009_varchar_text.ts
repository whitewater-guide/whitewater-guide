...
const VIEWS = ['gauges', 'groups', 'media', 'sections', 'rivers', 'regions', 'sources', 'tags', 'points'];
...
const alterMany = async (db: Knex, toText: boolean) => {
  ...
  for (const [table, columns] of toTextByTable) {
    await db.schema.alterTable(table, (tbl) => {
      const typeFunc: ((name: string) => ColumnBuilder) = toText ? tbl.text.bind(tbl) : tbl.string.bind(tbl);
      for (const column of columns) {
        const [columnName, notNull] = column.split('#');
        if (notNull) {
          typeFunc(columnName).notNullable().alter();
        } else {
...
export const up = async (db: Knex) => {
  ...
  await db.schema.alterTable('rivers_translations', (tbl) => {
    tbl.specificType('alt_names', 'text[]').notNullable().defaultTo('{}').alter();
  });
  ...
  await db.schema.alterTable('points', (tbl) => {
    tbl.string('kind').notNullable().alter();
  });
...
export const down = async (db: Knex) => {
  ...
  await db.schema.alterTable('rivers_translations', (tbl) => {
    tbl.specificType('alt_names', 'varchar[]').notNullable().defaultTo('{}').alter();
  });
  await db.schema.alterTable('sections_translations', (tbl) => {
    tbl.specificType('alt_names', 'varchar[]').notNullable().defaultTo('{}').alter();
  });
  ...
  await db.schema.alterTable('points', (tbl) => {
    tbl.text('kind').notNullable().alter();
  });
