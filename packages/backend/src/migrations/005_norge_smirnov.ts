...
const upsertRivers = async (db: Knex, riverNames: string[], region: any): Promise<Map<string, string>> => {
  const riverIds: Map<string, string> = new Map<string, string>();
  for (const riverName of riverNames) {
    const exists = await db.select('river_id').from('rivers_translations')
      .where('name', 'ilike', riverName).first();
    if (exists) {
    ...
    if (inserted) {
      ...
      await db.table('rivers').update({ import_id: 'norway_smirnov_1' }).where({ id: inserted.id });
    }
  }
  return riverIds;
};

const upsertSection = async (db: Knex, section: KMLSection, riverId: string) => {
  const rawSection = {
    river: { id: riverId },
  ...
  const result: any = await rawUpsert(db, 'SELECT upsert_section(?, ?)', [rawSection, 'en']);
  if (result) {
    await db.table('sections').update({ import_id: 'norway_smirnov_1' }).where({ id: result.id });
  }
};
