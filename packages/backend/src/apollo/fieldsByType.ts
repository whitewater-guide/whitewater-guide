...
const isFragmentSpread = (node: SelectionNode): node is FragmentSpreadNode => node.kind === 'FragmentSpread';
...
const inspectSelections = (selections: SelectionNode[], type: GraphQLNamedType, fragments: Fragments, acc: Acc) => {
  selections.forEach((subField) => {
    if (isFieldNode(subField)) {
      ...
      const fragmentSelections: SelectionNode[] = get(fragment, 'selectionSet.selections', []);
...
export const fieldsByType = (info: GraphQLResolveInfo, result?: Acc) => {
  ...
  const { operation: { operation, selectionSet }, schema, fragments } = info;
