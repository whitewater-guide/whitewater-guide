...
import { dataURLtoFile, FileWithPreview } from '../../utils/files';
import { ImageUploader } from './ImageUploader';

rewiremock(() => require('@whitewater-guide/clients'))
  .callThrough()
  .with({
    useUploadLink: () => ({
...
const testImgFile = dataURLtoFile(png80x101raw, '80x101.png');
const png80x101: FileWithPreview = {
  ...testImgFile,
  preview: window.URL.createObjectURL(testImgFile),
};

storiesOf('ImageUploader', module)
  .addDecorator((story) => <div style={{ display: 'block' }}>{story()}</div>)
  .add('no data', () => {
    (clients as any).useUploadLink = () => ({
      uploading: false,
  ...
  .add('uploading', () => {
    return (
      <ImageUploader
        ...
        value={{
          id: 'a1',
          status: LocalPhotoStatus.UPLOADING,
          file: png80x101,
          resolution: [80, 101],
        }}
        title="Banner"
    ...
    return (
      <ImageUploader
        ...
        value={{
          ...
          url: 'http://localhost:6006/120x87.png',
          resolution: [120, 87],
        }}
        title="Banner"
        onChange={action('onChange')}
