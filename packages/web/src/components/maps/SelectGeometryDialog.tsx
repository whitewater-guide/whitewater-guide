...
export class SelectGeometryDialog extends React.PureComponent<Props, State> {
  ...
  render() {
    const disabled = this.state.points.length < MIN_POINTS[this.props.drawingMode];
    const actions = [
      <RaisedButton key="cancel" label="Cancel" onClick={this.props.onClose} />,
      <RaisedButton primary key="submit" disabled={disabled} label="Submit" onClick={this.onSubmit} />,
    ];
    return (
      <Dialog
        open
