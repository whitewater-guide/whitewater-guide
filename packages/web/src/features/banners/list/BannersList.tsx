...
export default class BannersList extends React.PureComponent<BannerListProps> {
  ...
  onBannerClick = (id: string) => this.props.history.push(`/banners/${id}/settings`);
  ...
  renderPlacement: TableCellRenderer = ({ rowData: { placement } }) => (
    <p>
      {upperFirst(snakeCase(placement)).replace(/_/g, ' ')}
    </p>
  );

  renderGroups: TableCellRenderer = ({ rowData: { groups } }) => (
    <ul>
      {groups.nodes.map(({ id, name }: NamedNode) => (<li key={id}>{name}</li>))}
    </ul>
  );

  renderRegions: TableCellRenderer = ({ rowData: { regions } }) => (
    <ul>
      {regions.nodes.map(({ id, name }: NamedNode) => (<li key={id}>{name}</li>))}
    </ul>
  );
  ...
  render() {
    return (
      <ResourcesListCard
        ...
        rowHeight={188}
      >
        <Column width={150} flexGrow={1} label="Name" dataKey="name" style={styles.multiline} />
        <Column width={100} label="Slug" dataKey="slug" style={styles.multiline}/>
        <Column width={100} label="Placement" dataKey="placement" cellRenderer={this.renderPlacement} style={styles.multiline} />
        <Column width={528} label="Preview" dataKey="source" cellRenderer={this.renderBanner} />
        <Column width={100} flexGrow={1} label="Groups" dataKey="groups" cellRenderer={this.renderGroups} />
        <Column width={100} flexGrow={1} label="Regions" dataKey="regions" cellRenderer={this.renderRegions} />
        <Column width={40} label="Priority" dataKey="priority" />
        <AdminColumn width={45} label="Enabled" dataKey="enabled" cellRenderer={this.renderEnabled} />
