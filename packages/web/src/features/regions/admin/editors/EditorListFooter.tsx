  ...
  render() {
    ...
    return (
      <TableRow>
        ...
        <TableRowColumn>
          <IconButton disabled={disabled} iconClassName="material-icons" onClick={this.onAdd}>
            add
          </IconButton>
        </TableRowColumn>
      </TableRow>
...
const EditorListFooterWithMutation: React.StatelessComponent<Props> = ({ regionId }) => (
  <Mutation mutation={ADD_EDITOR_MUTATION} refetchQueries={['regionEditors']}>
    {
      (addEditor, { loading }) => (
        <EditorListFooter addEditor={addEditor} loading={loading} regionId={regionId} />
      )
    }
  </Mutation>
);
