...
export class RegionsList extends React.PureComponent<RegionsListProps> {
  ...
  renderActions: TableCellRenderer = ({ rowData: { id: regionId, editable } }) => {
    return (
      <ClickBlocker>
        {
  ...
  render() {
    return (
      ...
      >
        ...
        <AdminColumn width={50} label="Premium" dataKey="premium" cellRenderer={this.renderPremium} />
        <AdminColumn width={50} label="Visible" dataKey="hidden" cellRenderer={this.renderVisible} />
        <Column width={150} label="Actions" dataKey="hidden" cellRenderer={this.renderActions} />
      </ResourcesListCard>
    );
  }
}
