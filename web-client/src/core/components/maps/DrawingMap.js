...
import { arrayToGmaps, isValidLat, isValidLng } from '../../../commons/utils/GeoUtils';
...
export default class DrawingMap extends React.Component {
  ...
  onClick = ({ lat, lng, initial }) => {
    ...
    if (!initial) {
      const newPoints = [...this.state.points, [lng, lat]];
      this.setState({ points: newPoints });
      this.props.onChange(newPoints);
    }

    marker.addListener('dragend', ({ latLng }) => {
      const newPoints = _.cloneDeep(this.state.points);
  ...
  onLoaded = ({ map, maps }) => {
    ...
    const MVCArrayBinder = function (mvcArray) {
