...
func TestFilterMeasurements(t *testing.T) {
	...
	if !reflect.DeepEqual(FilterMeasurements(input, 0), expected) {
		t.Errorf("Expected to filter 0 flow + 0 level measurements, got %#v", FilterMeasurements(input, 0))
	}
	...
	if !reflect.DeepEqual(FilterMeasurements(input, 0), expected) {
		t.Errorf("Expected to sort from newest to oldest, got %#v", FilterMeasurements(input, 0))
	}
