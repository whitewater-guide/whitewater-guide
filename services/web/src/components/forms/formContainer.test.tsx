...
const mutateError = jest.fn((data: any) =>
  Promise.reject({ message: 'Some mutation error' }),
);
...
const detailsContainer = (loading: boolean, nullResult = false) =>
  withProps(({ language }) => ({
    data: {
...
const mutationContainer = (error: boolean) =>
  withProps({
...
it('should navigate on successful without explicit backPath', async () => {
  const history = createMemoryHistory({
    initialEntries: ['/foos/123/entitys/456/settings'],
  });
  history.replace = jest.fn();
  (graphql as jest.Mock)
...
it('should pass form error on mutation error', async () => {
  ...
  expect(receiver.prop('error')).toBe(
...
it('should receive language from query string', () => {
  const history = createMemoryHistory({
    initialEntries: ['/smth?language=es'],
  });
  const wrapped = mountThings(false, false, false, history);
  const receiver = wrapped.find(FormReceiver).first();
