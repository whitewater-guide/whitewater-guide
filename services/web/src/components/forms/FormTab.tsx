...
const FormTabView: React.StatelessComponent<Props & WithSyncErrors> = (
  props,
) => {
  const { form, label, fields, syncErrors, children, ...rest } = props;
  const hasErrors = Object.keys(syncErrors || {}).some((key) =>
    fields.includes(key),
  );
  const lbl = hasErrors ? (
...
export const FormTab: React.ComponentType<Props> & FakeMui = container(
  FormTabView,
);
FormTab.muiName = 'Tab';
