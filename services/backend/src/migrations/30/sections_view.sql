CREATE OR REPLACE VIEW sections_view AS
  WITH langs AS (
    ...
    SELECT
      ...
      sections.rating,
      (ST_AsGeoJSON(ST_QuantizeCoordinates(sections.shape) :: json)) AS shape,
      (ST_AsGeoJSON(ST_QuantizeCoordinates(st_startpoint(sections.shape :: geometry) :: geography) :: json)) AS put_in,
      (ST_AsGeoJSON(ST_QuantizeCoordinates(st_endpoint(sections.shape :: geometry) :: geography) :: json)) AS take_out,
      sections.created_at,
      sections.updated_at,
      sections.created_by,
      sections.gauge_id,
