...
export const up = async (db: Knex) => {
  ...
  // Make sku uniques
  await db.raw(
    'ALTER TABLE groups ADD CONSTRAINT groups_sku_unique UNIQUE (sku)',
  );
  await db.raw(
  ...
  await createTable(db, 'transactions', (table) => {
    ...
    table
      .specificType('platform', 'platform_type')
      .notNullable()
      .index();
  ...
  if (process.env.NODE_ENV !== 'test') {
    await db
      .table('groups')
      .insert({ id: ALL_REGIONS_GROUP, sku: 'group.all', all_regions: true });
    await db.table('groups_translations').insert({
      group_id: ALL_REGIONS_GROUP,
      language: 'en',
      name: 'All regions',
    });
  }
