...
import { REGION_ECUADOR, REGION_NORWAY } from '~/seeds/test/04_regions';
...
describe('resolvers chain', () => {
  ...
  it('should not pass editor who does not own both regions', async () => {
    const result = await runQuery(mutation, variables, fakeContext(EDITOR_NO));
    expect(result).toHaveGraphqlError(ApolloErrorCodes.FORBIDDEN);
  });

  it('should not pass editor who does not own river', async () => {
    const result = await runQuery(mutation, variables, fakeContext(EDITOR_GE));
    expect(result).toHaveGraphqlError(ApolloErrorCodes.FORBIDDEN);
  });
